...
const MovieCard = ({ id, poster_path, title, vote_average, release_date, adult }: Movie) => {
    return (
        <Link href={`/movie/${id.toString()}`} asChild>
            <TouchableOpacity className='w-[30%]'>
                ...
                <View className="flex-row items-center justify-start gap-x-1">
                    ...
                    <Text className="texr-xs text-white font-bold uppercase">{Math.round(vote_average / 2)}</Text>
                </View>
                <View className="flex-row items-center justify-between">
                    <Text className="text-xs text-light-300 mt-1 font-bold">
                        {release_date?.split('-')[0] || 'N/A'}
                    </Text>
                    {/* <Text className="text-xs font-medium text-light-300 uppercase">Movie</Text> */}
                    <Text className="text-xs font-extralight text-light-300 uppercase">{adult ? "Adult" : "Kids / Family"}</Text>
                </View>
            </TouchableOpacity>
        </Link>
    )
